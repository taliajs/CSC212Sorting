...
public class BubbleSortTest {
	...
	public void testIterative() {
		...
		ListADT<Integer> iterative = MergeSort.iterativeSort(sortMe);
		Assert.assertTrue(SortTestingHelpers.checkSorted(iterative, SortTestingHelpers.data.length));

		Random rand = new Random(13);
		// For good measure, let's shuffle it and sort it again to see if that works,
		// too.
		sortMe.shuffle(rand);
		ListADT<Integer> iterative2 = MergeSort.iterativeSort(sortMe);
		Assert.assertTrue(SortTestingHelpers.checkSorted(iterative, SortTestingHelpers.data.length));
